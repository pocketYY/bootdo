...
//设置全局变量保存摄像头编号修改前的值
var cameraCode;
var cameraCodeFirstChange = true;
//设置全局变量保存摄像头原始流地址修改前的值
var osUrl;
var osUrlFirstChange = true;
...
jQuery.validator.addMethod("cameraCodeRepeatCheck", function (value, element) {
    ...
    var result = true;
    //当基地编号发生改变时，判断是否重复
    if (cameraCode != undefined && value != cameraCode) {
        $.ajax({
            cache: true,
            type: "POST",
...
jQuery.validator.addMethod("osUrlRepeatCheck", function (value, element) {
    ...
    var result = true;
    //当摄像头编号发生改变时，判断是否重复
    if (osUrl != undefined && value != osUrl) {
        $.ajax({
            cache: true,
            type: "POST",
